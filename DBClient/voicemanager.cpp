...
void VoiceManager::connectLiveKit(const QString& url, const QString& token)
{
    ...
    _connect_thread = QThread::create([this, server_url, access_token]() {
        ...
        try
        {
            const bool connected = _room->connect(server_url, access_token, options);
...
{
    if (!_room || !_platform_audio)
        return;

    try
    {
        // PlatformAudio 使用 WebRTC 系统音频设备，并自带回声消除、降噪和自动增益。
        _audio_source = _platform_audio->createAudioSource();
        _audio_track = livekit::LocalAudioTrack::createLocalAudioTrack(
            "microphone", _audio_source);

        livekit::TrackPublishOptions options;
        options.source = livekit::TrackSource::SOURCE_MICROPHONE;

        const auto participant = _room->localParticipant().lock();
        if (!participant)
            throw std::runtime_error("本地参与者不可用");

        participant->publishTrack(_audio_track, options);
...
{
    // 连接回调来自 LiveKit 线程，不能在这里初始化音频设备或发布轨道。
