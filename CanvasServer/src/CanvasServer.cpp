...
int main()
{
    ...
    Logger::Init("CanvasServer", "logs/CanvasServer.log");
    LOG_INFO_CTX("CanvasServer::main", "服务启动");
    try
    {
        // 初始化配置
        auto& cfg = ConfigMgr::Inst();
        LOG_INFO_CTX("CanvasServer::main", "配置加载完成");

        // 初始化 Redis
        RedisMgr::getInstance();
        LOG_INFO_CTX("CanvasServer::main", "RedisMgr 初始化完成");

        // 初始化线程池 (AsioIOServicePool)
        // 要在 Server 启动前把 IO 线程跑起来
        AsioIOServicePool::getInstance();
        LOG_INFO_CTX("CanvasServer::main", "IO 线程池初始化完成");

        // 初始化业务逻辑系统 (LogicSystem 构造时会启动业务处理线程)
        LogicSystem::getInstance();
        LOG_INFO_CTX("CanvasServer::main", "LogicSystem 初始化完成");

        // 初始化管理器
        SessionMgr::getInstance();
        RoomMgr::getInstance();
        LOG_INFO_CTX("CanvasServer::main", "SessionMgr 和 RoomMgr 初始化完成");
        ...
        signals.async_wait([&io_context](const boost::system::error_code& error, int signal_number)
            {
                if (!error)
                {
                    LOG_INFO_CTX("CanvasServer::main", "收到停止信号 signal=" << signal_number);
        ...
        LOG_INFO_CTX("CanvasServer::main", "TCP 服务监听 port=" << port);

        // 阻塞主线程，处理连接请求
        // 之后的 Read/Write 操作会由 AsioIOServicePool 里的线程去跑，不占用这里
        io_context.run();

        LOG_INFO_CTX("CanvasServer::main", "服务正常停止");
    }
    catch (const std::exception& e)
    {
		LOG_ERROR_CTX("CanvasServer::main", "服务异常: " << e.what());
        Logger::Shutdown();
        return -1;
    }
    catch (...)
    {
		LOG_ERROR_CTX("CanvasServer::main", "服务发生未知异常");
        Logger::Shutdown();
        return -1;
    }
	Logger::Shutdown();
